...
output_file = '/bgfs/alee/LO_LAB/Personal/Daisong/CPT6/ICLE/CNV/IDC_data_CNA_gistic.txt'
...
def main():
    try:
        ...
        # Filter for Invasive ductal carcinoma
        print("Step 2: Filtering for Invasive ductal carcinoma samples...")
        idc_samples = tcga_samples[tcga_samples.iloc[:, 1] == "Infiltrating ductal carcinoma"]
        
        # If no samples found with "Infiltrating ductal carcinoma", try alternative naming
        if idc_samples.empty:
            idc_samples = tcga_samples[tcga_samples.iloc[:, 1] == "Invasive ductal carcinoma"]
            
        if idc_samples.empty:
            print("Error: No samples with 'Invasive ductal carcinoma' or 'Infiltrating ductal carcinoma' found!")
            sys.exit(1)
        
        # Extract IDs from the first column
        idc_ids_raw = idc_samples.iloc[:, 0].tolist()
        print(f"Found {len(idc_ids_raw)} samples with Invasive ductal carcinoma")
        ...
        print(f"Processed {len(idc_ids_processed)} variations of sample IDs for matching")
        ...
        for col in cn_columns[1:]:
            ...
            for idc_id in idc_ids_processed:
                if idc_id == col or col.startswith(idc_id + '-') or idc_id.startswith(col + '-'):
                    filtered_columns.append(col)
                    matched_ids.append(idc_id)
                    break
        
        if len(filtered_columns) <= 1:
            print("Error: No matching columns found in copy number data!")
            print("Sample IDs in TCGA_sample.csv (first 5):", idc_ids_raw[:5])
        ...
        print(f"Original samples: {len(idc_ids_raw)}, Matched samples in output: {len(filtered_columns) - 1}")
