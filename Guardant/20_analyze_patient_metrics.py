...
def analyze_patient_metrics(df, cohort_name):
    ...
    # Get unique patients
    unique_patients = df.drop_duplicates(subset=['GH_ID'])
    ...
    tumor_data = pd.DataFrame({
        'Patient_ID': unique_patients['GH_ID'],
        'Original_Tumor_Fraction': unique_patients['Tumor Fraction'],
        'Processed_Tumor_Fraction': tumor_fractions
    }).sort_values('Patient_ID')
    tumor_data.to_csv(f'{cohort_name}_tumor_fractions.csv', index=False)
    print(f"  Tumor fraction data saved to: {cohort_name}_tumor_fractions.csv")
...
# Save to file
with open('patient_metrics_analysis.txt', 'w') as f:
    f.write(output_text)

print("Full analysis report saved to: patient_metrics_analysis.txt") 