...
def analyze_test_results(df, subtype):
    ...
    patient_results = df.groupby('Effective Patient ID').agg({
        'Aleration Detected?': ['count', lambda x: all(x == 'False')],
        'Total Test': 'max'
    }).reset_index()
    ...
    # Get breakdown by number of tests
...
print(f"Patients with all negative tests: {idc_negative}")
...
print(f"Patients with all negative tests: {ilc_negative}")
...
output_file = 'patient_test_results.csv'
