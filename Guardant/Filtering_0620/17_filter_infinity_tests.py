...
# Save filtered data
idc_infinity.to_csv('IDC_genomic_infinity.csv', index=False)
ilc_infinity.to_csv('ILC_genomic_infinity.csv', index=False)

# Generate summaries
def generate_summary(df, cohort):
    summary = {
        f'Total {cohort} Infinity Tests': len(df),
        f'Unique {cohort} Patients': len(df['GH_ID'].unique()),
        'Gene Mutations': {
...
idc_summary.to_csv('IDC_infinity_summary.csv', index=False)
ilc_summary.to_csv('ILC_infinity_summary.csv', index=False)
