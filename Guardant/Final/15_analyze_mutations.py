...
# --- Configuration ---
ILC_GENOMIC_FILE = 'ILC_genomic_corrected.csv'
IDC_GENOMIC_FILE = 'IDC_genomic_corrected.csv'
PATIENT_ID_COLUMN = 'Effective Patient ID'
TEXT_OUTPUT_FILE = 'mutation_analysis_report_corrected.txt'
CSV_OUTPUT_FILE = 'mutation_details_corrected.csv'
...
def analyze_mutations(df, cohort_name):
    ...
    for gene in key_genes:
        gene_count = df[df['Gene'].apply(lambda x: contains_gene(x, gene))][PATIENT_ID_COLUMN].nunique()
...
try:
print("\nReading data files...")
    ilc_genomic = pd.read_csv(ILC_GENOMIC_FILE, encoding='latin-1')
    idc_genomic = pd.read_csv(IDC_GENOMIC_FILE, encoding='latin-1')
    print("...done")
except FileNotFoundError as e:
    # Temporarily restore stdout to print the error to the console
...
def generate_mutation_details(df, cohort):
    if df.empty:
        return pd.DataFrame(columns=['Gene', 'Total_Mutations', 'Cohort', 'Unique_Patients'])
    ...
    gene_counts['Unique_Patients'] = df.groupby('Gene')[PATIENT_ID_COLUMN].nunique().reindex(gene_counts['Gene']).values
    return gene_counts
...
all_details.to_csv(CSV_OUTPUT_FILE, index=False)
print(f"Detailed mutation report saved to: {CSV_OUTPUT_FILE}")
...
print(f"Full analysis report saved to: {TEXT_OUTPUT_FILE}") 