...
def analyze_mutations(df, cohort_name, summary_buffer, detail_buffer):
    ...
    def analyze_gene_mutations(gene):
        ...
        unique_patients = len(gene_data['Effective Patient ID'].unique())
        ...
        for _, row in gene_data.iterrows():
            ...
            if is_fusion(gene_name, gene):
                fusions.append({
                    'name': gene_name,
                    'patient_id': row['Effective Patient ID']
                })
                fusion_patients.add(row['Effective Patient ID'])
...
def main():
    ...
    print("\nCombined (ILC + IDC) Analysis:", file=summary_buffer)
    total_patients_combined = idc_patients + ilc_patients
    print(f"Total unique patients: {total_patients_combined}", file=summary_buffer)
    print(f"Total tests: {idc_tests + ilc_tests}", file=summary_buffer)
    print(f"Total patients with multiple tests: {idc_multiple + ilc_multiple}", file=summary_buffer)
    ...
    detail_output = detail_buffer.getvalue()
    
    with open('analysis_summary_report_corrected.txt', 'w') as f:
        f.write(summary_output)
        
    with open('analysis_detail_report_corrected.txt', 'w') as f:
        f.write(detail_output)
    
    print("Script finished.")
    print("Summary report saved to: analysis_summary_report_corrected.txt")
    print("Detailed report saved to: analysis_detail_report_corrected.txt")
