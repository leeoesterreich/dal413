...
# ============ Data Preparation ============
file_path = "/ix1/alee/LO_LAB/Personal/Daisong/Projects/APOLLO/Extraction/Concentration_summary_4.7.csv"

# Read CSV file
df = pd.read_csv(file_path)

# Clean sample names by removing parentheses and numbers
df['Samples'] = df['Sample'].apply(lambda x: re.sub(r'\s*\([^)]*\)', '', x).strip())

# Handle "<100" values
df['Concentration'] = df['Concentration(ng/mL)'].apply(lambda x: 100 if isinstance(x, str) and x == "<100" else float(x))
...
# ============ Final Formatting ============
ax.set_title("ILC ctDNA concentration summary", pad=20)
...
plt.tight_layout()
plt.savefig('concentration_plot_ILC.png', dpi=300, bbox_inches='tight')
plt.close() 