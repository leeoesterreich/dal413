...
blood_draw = pd.read_csv('/ix1/alee/LO_LAB/Personal/Daisong/Projects/APOLLO/shWGS/ichorCNA/500kb/results/Blood_draw.csv')
summary = pd.read_csv('/ix1/alee/LO_LAB/Personal/Daisong/Projects/APOLLO/shWGS/ichorCNA/500kb/results/summary.csv')

print("Successfully read Blood_draw.csv with %d rows" % len(blood_draw))
print("Successfully read summary.csv with %d rows" % len(summary))
...
for i in range(1, max_draws + 1):
    columns.extend([get_progression_name(i), 'Tumor_fraction_500kb bin_%d' % i])
...
for _, row in blood_draw.iterrows():
    ...
    for i in range(1, max_draws + 1):
        ...
        tf_col = 'Tumor_fraction_500kb bin_%d' % i
...
rename_dict = {}
for i in range(1, max_draws + 1):
    old_col = 'Tumor_fraction_500kb bin_%d' % i
    rename_dict[old_col] = 'Tumor_fraction_500kb bin'
...
# Save to file
output_path = '/ix1/alee/LO_LAB/Personal/Daisong/Projects/APOLLO/shWGS/ichorCNA/500kb/results/TFx_hg38_500kb.csv'
final_df.to_csv(output_path, index=False)

print("\nFinal output shape: (%d, %d)" % final_df.shape)
print("Data has been saved to: %s" % output_path)
