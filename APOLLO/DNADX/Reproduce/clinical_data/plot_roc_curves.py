...
def plot_ghi_rs_roc_curve():
    """Plot ROC curve for the GHI_RS model with OncotypeDX title"""
    ...
    # Get the signature name from the model path
    signature = 'GHI_RS_Model_NJEM.2004_PMID.15591335'
    
    # Try to get the corresponding target values
    try:
        y_test = test_signature_score.loc[signature].values
    except KeyError:
        # If signature not found, use the first row as a fallback
        print(f"Warning: {signature} not found in test_signature_score. Using first row instead.")
        y_test = test_signature_score.iloc[0].values
    ...
    try:
        ...
        # If signature not found, use the first row as a fallback
        print(f"Warning: {signature} not found in metabric_signature_score. Using first row instead.")
        y_metabric = metabric_signature_score.iloc[0].values
    ...
    # Use OncotypeDX as the title
    plt.title('OncotypeDX')
    ...
    # Save the plot
    plt.savefig('roc_curves_oncotypedx.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("ROC curve for GHI_RS model saved as 'roc_curves_oncotypedx.png'")
...
if __name__ == "__main__":
    # Call the new function to plot the GHI_RS ROC curve with OncotypeDX title
    plot_ghi_rs_roc_curve() 