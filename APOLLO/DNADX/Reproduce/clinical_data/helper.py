...
def fisher_test(score, CN):
    ...
    # Only run Fisher's exact test if table is 2x2
    if table.shape == (2, 2):
...
def sig_CN_test(score, CN_score, CN_gain, CN_loss, subtype_variable=None):
    ...
    Always include subtype_variable in the linear model if provided.
    """
    results = []
    for gene in CN_score.index:
        ...
        # Linear model
        lm_pos = lm_neg = np.nan
        X = pd.DataFrame({'CN': CN})
        if subtype_variable is not None:
            for col in ['Basal', 'Her2', 'LumA', 'LumB', 'Normal']:
                if col in subtype_variable.columns:
                    X[col] = subtype_variable[col]
        y = score
        # Align indices
        X = X.loc[y.index]
        try:
