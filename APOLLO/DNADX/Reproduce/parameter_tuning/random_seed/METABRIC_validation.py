...
def load_all_signatures():
    ...
    metrics_df = pd.read_csv("/ix1/alee/LO_LAB/Personal/Daisong/Projects/APOLLO/DNADX/Reproduce/parameter_tuning/random_seed/model_metrics.csv")
    # Only take the first 100 signatures
    metrics_df = metrics_df.head(100)
    print("Found {} signatures in total".format(len(metrics_df)))
    return metrics_df
...
def plot_auc_comparison(results_df):
    ...
    # Save plot
    plt.savefig('results_rs10/validation_summary/auc_comparison.png')
    plt.close()
