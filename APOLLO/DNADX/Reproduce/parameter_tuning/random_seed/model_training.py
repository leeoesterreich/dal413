...
def plot_model_performance(y_true, y_pred, signature_name, correlation, mse):
    ...
    plt.savefig(f'results_rs10/plots/{safe_name}_performance.png')
    plt.close()
...
def build_prediction_model(signature_score, segment_score, target_signature):
    ...
    dump(model_info, f'results_rs10/models/{safe_name}_model.joblib')
    print(f"Saved model and preprocessing information to results_rs10/models/{safe_name}_model.joblib")
    ...
    # Save metrics
    metrics_file = 'results_rs10/plots/model_metrics.csv'
