...
def plot_model_performance(y_true, y_pred, signature_name, correlation, mse):
    ...
    plt.savefig(f'results_rs100/plots/{safe_name}_performance.png')
    plt.close()
...
def build_prediction_model(signature_score, segment_score, target_signature):
    ...
    dump(model_info, f'results_rs100/models/{safe_name}_model.joblib')
    print(f"Saved model and preprocessing information to results_rs100/models/{safe_name}_model.joblib")
    ...
    # Save metrics
    metrics_file = 'results_rs100/plots/model_metrics.csv'
...
def train_all_signatures():
    """Train models for all signatures"""
    # Load data
    ...
    for signature in signature_score.index:
        try:
            # Build model
            model, test_correlation, test_mse, test_auc = build_prediction_model(
                signature_score, segment_score, signature
            )
    ...
    # Save results
    results_df.to_csv('results_rs100/plots/model_metrics.csv', index=False)
    
    # Print summary statistics
    print("\nSummary Statistics:")
    print("\nTest Correlation:")
    print(results_df['test_correlation'].describe())
    print("\nTest MSE:")
