...
def plot_basal_signaling_bars_from_excel(excel_file):
    """Plot feature landscape for Basal signaling from Excel file using bar plot"""
    
    print(f"Reading Excel file: {excel_file}")
    df = pd.read_excel(excel_file, sheet_name='Sheet1')
    print(f"Successfully read sheet with {len(df)} rows and {len(df.columns)} columns")
    
    # Look for the Basal signaling row
    basal_row = None
    for idx, row in df.iterrows():
        row_name = str(row[df.columns[0]])  # First column contains row names
        if 'GSEA_Median_GP17_Basal_signaling' in row_name:
            basal_row = idx
            print(f"Found Basal signaling row at index {idx}: {row_name}")
            break
    
    if basal_row is None:
        print("Could not find GSEA_Median_GP17_Basal_signaling row")
        print("Available rows (first 10):")
        for idx, row in df.head(10).iterrows():
            print(f"  {idx}: {row[df.columns[0]]}")
        return
    
    # Extract weights for this row (skip first column which is the row name)
    feature_columns = df.columns[1:]  # All columns except first
    weights = df.iloc[basal_row, 1:].values  # All values except first column
    
    print(f"Found {len(weights)} features")
    
    # Find non-zero weights
    non_zero_mask = weights != 0
    non_zero_features = feature_columns[non_zero_mask]
    ...
    ax.set_title('GSEA Basal Signaling Feature Landscape (Bar Plot from Excel)', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('GSEA_Basal_signaling_Excel_bars.png', dpi=300, bbox_inches='tight')
    print("Plot saved as: GSEA_Basal_signaling_Excel_bars.png")
    ...
    for i, idx in enumerate(top_indices):
        print(f"{i+1:2d}. {feature_names[idx][:50]}... : {final_weights[idx]:.6f}")

if __name__ == "__main__":
    excel_file = "41467_2019_13588_MOESM4_ESM_Elastic_Net_gene_signatures.xlsx"
    plot_basal_signaling_bars_from_excel(excel_file) 